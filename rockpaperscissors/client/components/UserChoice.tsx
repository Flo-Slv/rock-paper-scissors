...
const UserChoice = () => {
  ...
  const setUserChoice = useStore((state) => state.setUserChoice);

  return (
    <div className="flex flex-row gap-5 mt-16 md:mt-24">
      <button
        className={clsx(
          "rounded-xl border border-gray-800 bg-gray-900",
          "w-[100px] h-[100px] lg:w-[120px] lg:h-[120px] p-4 shadow-xl sm:p-6 lg:p-8",
          "group",
          "flex justify-center items-center",
          `${nickName ? "cursor-pointer" : "cursor-not-allowed"}`
        )}
        onClick={() => setUserChoice("rock")}
        disabled={nickName ? Boolean(false) : Boolean(true)}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="48"
          height="48"
          className="group-hover:fill-indigo-400 fill-indigo-300"
        >
